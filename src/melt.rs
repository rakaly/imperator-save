use crate::{
    flavor::ImperatorFlavor, tokens::TokenLookup, zip_index, Extraction, FailedResolveStrategy,
    ...
    fn convert(&self, input: &[u8], writer: &mut Vec<u8>) -> Result<(), ImperatorError> {
        ...
        while let Some(token) = tokens.get(token_idx) {
            ...
            match token {
                ...
                BinaryToken::I32(x) => {
                    if let Some(date) = ImperatorDate::from_i32(*x) {
                        writer.extend_from_slice(date.imperator_fmt().as_bytes());
                    } else {
