...
/// Fetch an imperator save file. Save files can be quite large, so the save files are not stored in the
...
pub fn request<S: AsRef<str>>(input: S) -> Vec<u8> {
    ...
    if cache.exists() {
        ...
        let url = format!("https://imperator-test-cases.s3.us-west-002.backblazeb2.com/{}", reffed);
